...
Function printTaxRecord()
	...
	For recordNumber=1 to 5
		...
		End If
	Next
	'printTaxRecordRS4()    'this will use the Function below to handle the objRS4 when there are more tax reciepts than 5
End Function
'code below added 6/13/05 to handle the possiblity of more than 5 tax reciepts in the database Added new record set
' to handle the additional tax reciepts in the database . Some counties may have this many
Function printTaxRecordRS4()
		...
		If objRS4("RCBTDT" & recordNumber) = 0 Then
			...
			Else
				Response.Write("<tr valign='top'>")
					...
					Response.Write("</tr>")
				printTaxValues4(recordNumber)'This uses the objRS4
			End If
	Next
End Function
	...
	For subrecordNumber=2 to 6
		...
		If (objRS3(("RCSAA"& subrecordNumber & recordNumber )) = 0) and (objRS3(("RCAMT" & subrecordNumber & recordNumber))=0) Then
			...
			Response.Write("<tr valign='top'>")
				...
				Response.Write("<td width='80' class='rText'>&nbsp;</td>")
				Response.Write("<td width='80' class='rText'>&nbsp;</td>")
				If subrecordnumber => 5 and recordNumber = 1 Then
					Response.Write("<td width='80' class='rText'>&nbsp;</td>")
				End If
				Response.Write("<td width='80' class='rText' align='right'>&nbsp;</td>")
			Response.Write("</tr>")
				...
				Response.Write("<tr valign='top'>")
					...
					Response.Write("<td width='80' class='rText'>&nbsp;</td>")
					Response.Write("<td width='80' class='rText'>&nbsp;</td>")
					If subrecordnumber => 4 and recordNumber = 1 Then
						Response.Write("<td width='80' class='rText'>&nbsp;</td>")
					End If
					'Response.Write("<td width='80' class='rText'>"& subrecordnumber & recordNumber & "</td>")
				If (objRS3(("RCSAA3" & recordNumber )) = 0) or (objRS3(("RCAMT3" & recordNumber )) = 0) Then
				else
						Response.Write("<td width='80' class='rText'>&nbsp;</td>")
				End if
...
'the following Function is a repeat of the one above with the only exception of the objRS is 4 not 3 as above
'
...
'					End If
'					'Response.Write("<td width='80' class='rText'>"& subrecordnumber & recordNumber & "</td>")
'				If (objRS4(("RCSAA3" & recordNumber )) = 0) or (objRS4(("RCAMT3" & recordNumber )) = 0) Then
'					else
'						Response.Write("<td width='80' class='rText'>&nbsp;</td>")
'				End if
'				Response.Write("<td width='80' class='rText'>Total</td>")
...
Set objCommand = Nothing
			...
			dim intHyphenPos, strTemp1, strTemp2, strTemp3, intEndOfTemp1, intEndOfTemp2, strPIDnew
			intEndOfTemp1 = Instr(strPID,"-")
			strTemp1 = mid(strPid, 1, intEndOfTemp1-1)
			strTemp2 = mid(strPID, intEndofTemp1+1, len(strPID))
			intEndOfTemp2 = Instr(strTemp2,"-")
			strTemp3 = mid(strTemp2, 1, intEndOfTemp2-1)
			strTemp4 = mid(strTemp2, intEndOfTemp2+1, len(strPID))

			strPIDnew = Trim(strTemp1)  + Trim(strTemp3) + Trim(strTemp4)
...
<%
			...
			'End If
			'If cName = "Nobles" Then
			'	Response.Write("<a href='http://www.co.nobles.mn.us/' class='tlink'>Nobles County Home Page</a>")
			'End If
			'If cName = "Swift" Then
			'	Response.Write("<a href='http://www.co.swift.mn.us/' class='tlink'>Swift County Home Page</a>")
			'End If
